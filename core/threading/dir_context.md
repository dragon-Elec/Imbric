...
!Rule: [Signal only, never direct call from worker] - Reason: GioTask runs on QThreadPool threads; direct Qt UI mutation = crash. All results surface via resultReady / errorOccurred signals.
...
/DNA/: `enqueue(task_id, func, ...)` -> [push heapq, ++pending_batches] -> `_process_queue()` -> if active < max: `GioTask(func).run()` -> [success: em:resultReady | error: em:errorOccurred] -> --active -> if()batch_done: em:allTasksDone -> `_process_queue()`
...
!Caveat: `WorkerPool` is a backwards-compat alias for `GioWorkerPool`.
