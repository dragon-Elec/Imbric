    ...
    def clear(self, batch_id: str | None = None) -> None:
        with QMutexLocker(self._mutex):
            if batch_id:
                ...
                for evt in self._batch_events.values():
                    evt.set()
    ...
    def _process_queue(self) -> None:
        with QMutexLocker(self._mutex):
            while self._queue and self._active_count < self.max_concurrent:
                ...
                on_end = lambda t_id, r, e, c, bid=batch_id: self._on_task_end(
                    t_id, bid, r, e, c
                )
                on_prog = lambda t_id, c_p, t_p: self.progressUpdated.emit(
                    t_id, c_p, t_p
                )

                cancel_evt = (
                    self._batch_events.get(batch_id, self._global_cancel)
                    if batch_id
