"""
GIO Trash Operations - Runnable implementations.
Extracted from core/trash_workers.py.
"""
...
class SendToTrashRunnable(GIOOperationRunnable):
...
class RestoreFromTrashRunnable(GIOOperationRunnable):
    ...
    def run(self):
        ...
        try:
            ...
            if e.code == Gio.IOErrorEnum.EXISTS:
                ...
                if self.job.rename_to:
                    parent_gfile = _make_gfile(original_path).get_parent()
                    final_dest_path = _gfile_path(
                        parent_gfile.get_child(self.job.rename_to)
                    )
...
class ListTrashRunnable(GIOOperationRunnable):
...
class EmptyTrashRunnable(GIOOperationRunnable):
    ...
    def run(self):
        ...
        try:
            ...
            if e.code != Gio.IOErrorEnum.CANCELLED:
                self.signals.operationError.emit(
                    self.job.transaction_id, self.job.id, "empty", "Trash", str(e), None
                )
            self.emit_finished(False, str(e))
        except Exception as e:
            msg = str(e)
            self.signals.operationError.emit(
                self.job.transaction_id, self.job.id, "empty", "Trash", msg, None
            )
            self.emit_finished(False, msg)
    ...
    def _empty_trash(self) -> int:
        ...
        enumerator = trash_root.enumerate_children(
            "standard::name", Gio.FileQueryInfoFlags.NONE, self.job.cancellable
        )
