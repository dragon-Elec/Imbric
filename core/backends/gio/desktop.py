"""
GIO Desktop Integration - XDG dirs, bookmarks, MIME factory.
Moved from core/gio_bridge/desktop.py
"""
...
def get_special_dirs() -> dict:
    ...
    _SPECIAL_DIR_CACHE = {str(Path.home()): ("Home", "user-home-symbolic")}
...
def resolve_identity(raw_path: str) -> str:
    """Background Worker: Resolves a raw path to its canonical GIO parsed name."""
    if not raw_path:
        return ""
...
def get_breadcrumb_segments(
    path: str, active_path: str, fast_mode: bool = False
) -> list:
    ...
    if segments:
        ...
        if not segments[0]["icon"]:
            ...
            segments[0]["icon"] = (
                "user-home-symbolic" if is_home_strict else "drive-harddisk-symbolic"
            )
            ...
            with open(path, "r") as f:
                for line in f:
                    ...
                    try:
                        ...
                        items.append(
                            {"name": name, "path": parsed_path, "icon": "folder"}
                        )
                    except Exception:
                        pass
        except Exception:
...
class QuickAccessBridge(QObject):
    ...
    def _setup_trash_monitor(self):
        try:
            ...
            self._monitor = trash_file.monitor_directory(
    ...
    def _rebuild_items_task():
        ...
        home_path = Path.home()
        items.append(
            {"name": "Home", "path": str(home_path), "icon": "home", "type": "standard"}
        )
        items.append(
            {
                "name": "Recent",
...
def create_desktop_mime_data(paths: list, is_cut: bool):
    """Robust QMimeData factory for GNOME/GTK desktop operations."""
