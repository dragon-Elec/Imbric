...
public final class Library implements Serializable {
    ...
    public void put(String name, Repository repository) {
        // Patch: Allow CLI-provided GIR files to override bundled ones.
        // If we are adding a namespace that already exists, remove the old repository.
        for (Namespace ns : repository.namespaces()) {
            repositories.entrySet().removeIf(entry -> 
                !entry.getKey().equals(name) &&
                entry.getValue().namespaces().stream().anyMatch(existingNs -> existingNs.name().equals(ns.name()))
            );
        }
