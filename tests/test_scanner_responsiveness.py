...
class AsyncVerifier(QObject):
    ...
    @Slot(str, list)
    def on_files(self, session_id: str, files: list):
        self.batch_count += 1
        dir_count = sum(1 for f in files if f.get("isDir", False))
        print(f"\n[Batch {self.batch_count}] Found {len(files)} files ({dir_count} dirs) [Session: {session_id[:8]}]")

    @Slot(str)
    def on_finished(self, session_id: str):
        self.scan_done = True
        print(f"\nScan Finished in {self.tick_count * 10}ms roughly! [Session: {session_id[:8]}]")
