...
class TestFileOperations:
    ...
    def setup_method(self, method):
        ...
        self.completed_jobs: List[Tuple[str, str, str]] = []
        self.started_jobs: List[Tuple[str, str, str]] = []
        ...
        self.file_ops.operationFinished.connect(self._on_finished_with_completion)
        self.file_ops.operationFinished.connect(self._on_finished)
    ...
    def setup(self):
        ...
        # Use local directory to ensure Trash support (system /tmp often doesn't support trash)
        base_dir = Path(os.getcwd()) / "tests" / "temp_artifacts"
        base_dir.mkdir(parents=True, exist_ok=True)

        self.test_dir = Path(tempfile.mkdtemp(prefix="imbric_test_", dir=str(base_dir)))
        ...
        print(
            f"   Created: file1.txt, file2.txt, file3.txt, large_file.bin, subdir/nested.txt"
        )

    def teardown(self):
        """Clean up test directory."""
        if self.test_dir is not None and self.test_dir.exists():
    ...
    def _on_finished_with_completion(
        self, tid, job_id, op_type, result_path, success, message, inv_payload=None
    ):
        if success:
            if (op_type, result_path, message) not in self.completed_jobs:
                self.completed_jobs.append((op_type, result_path, message))

    @Slot(str, str, str, str, bool, str, object)
    def _on_finished(
        self, tid, job_id, op_type, result_path, success, message, inv_payload=None
    ):
        # Universal finish signal
        if success:
            if (op_type, result_path, message) not in self.completed_jobs:
                self.completed_jobs.append((op_type, result_path, message))
        else:
            # For testing purposes, treat any non-success as an "error result"
            # if it hasn't already been reported by operationError
            if not self.errors or self.errors[-1][1] != result_path:
                self.errors.append((op_type, result_path, message))

        self.finished_count += 1

    @Slot(str, str, str, str, str, object)
    def _on_error(self, tid, job_id, op_type, path, error, conflict_data):
    ...
    def _wait_for_completion(self, expected_count=1, timeout_ms=5000):
        ...
        while self.finished_count < expected_count:
    ...
    def test_01_copy_file(self) -> bool:
        ...
        test_dir = cast(Path, self.test_dir)
        src = str(test_dir / "file1.txt")
        dest = str(test_dir / "file1_copy.txt")
        ...
        # Verify
        assert len(self.started_jobs) == 1, (
            f"Expected 1 start, got {len(self.started_jobs)}"
        )
        assert len(self.completed_jobs) == 1, (
            f"Expected 1 completion, got {len(self.completed_jobs)}"
        )
    ...
    def test_03_trash_file(self) -> bool:
        ...
        # Trash should work now that we are on a real filesystem
        if self.errors:
            error_msg = self.errors[0][2] if self.errors else ""
            print(f"  ❌ Trash failed: {error_msg}")
            return False
    ...
    def test_06_parallel_operations(self) -> bool:
        ...
        assert submit_time < 100, (
            f"Job submission took {submit_time}ms, should be < 100ms"
        )
        ...
        assert total == 5, (
            f"Expected 5 results, got {total} (completed: {len(self.completed_jobs)}, errors: {len(self.errors)})"
        )
    ...
    def test_08_active_job_count(self) -> bool:
        ...
        # Start copy
        job_id = self.file_ops.copy(
            str(large), str(self.test_dir / "big_copy_dest.bin")
        )
    ...
    def test_09_error_handling(self) -> bool:
        ...
        job_id = self.file_ops.copy(
            str(self.test_dir / "does_not_exist.txt"), str(self.test_dir / "dest.txt")
        )
