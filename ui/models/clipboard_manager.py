...
class ClipboardManager(QObject):
    ...
    def isCut(self) -> bool:
        ...
        # Fallback: If no GNOME marker, assume copy
        return False
    
    @Slot(result=list)
    def getCutPaths(self) -> List[str]:
        """
        Returns list of file paths if clipboard is in "cut" mode.
        Returns empty list if clipboard is in "copy" mode.
        """
        if self.isCut():
