...
class SectionItemsModel(QAbstractListModel):
    ...
    def data(self, index, role=Qt.DisplayRole):
        ...
        if role == self.IDENT_ROLE:
            return item.get("identifier")
        if role == self.USAGE_ROLE:
            return item.get("usage")
        if role == self.MOUNTED_ROLE:
            return item.get("isMounted", False)
        if role == self.UNMOUNT_ROLE:
            return item.get("canUnmount", False)
...
class SidebarModel(QAbstractListModel):
        ...
        self._sections = [
            {
                ...
                "type": "GRID",
                "collapsed": False,
                "actions": ["Add", "Grow", "Shrink", "Settings"],
    ...
    def data(self, index, role=Qt.DisplayRole):
        ...
        if role == self.TYPE_ROLE:
            return sec["type"]
        if role == self.COLLAPSED_ROLE:
            return sec["collapsed"]
        if role == self.ACTIONS_ROLE:
            return sec["actions"]
        if role == self.ITEMS_ROLE:
            return sec["items_model"]  # Return inner model directly!
